...
const WriteBoard = () => {
    ...
    //Next.js 13에서는 "next/navigation"에 있는 useRouter를 호출해서 사용해야 함
        ...
        <div>
            ...
            <div className="main">
                <div className={styles.WriteDiv}>
                    ...
                    <form className={styles.WriteForm} method="POST">
                        <input type="text" className={styles.writer} value={usernameCookie} readOnly />
                        <input type="text" className={styles.title} value={title} ref={titleRef} onChange={(e) => setTitle(e.target.value)} placeholder="여기에 제목을 입력하세요" />                    
                        <textarea className={styles.content} value={content} ref={contentRef} cols="100" rows="500" name="content" 
                        ...
                        <div className={styles.fileuploadForm}>
                            ...
                            <div className={styles.fileZone} ref={fileZoneRef} style={fileZoneStyle} 
                            ...
                            statusbarState.map((statusbar)=> ( statusbar.filename &&
                                <div className={statusbar.row} key={statusbar.filenum}>
                                    <div className={styles.filename}>{statusbar.filename}</div>
                                    <div className={styles.filesize}>{statusbar.filesize}</div>
                                    <div className={styles.b_delete} onClick={()=> fileDelete(statusbar.filenum)}>삭제</div>
                                </div>    
                            ))
                            }
                            </div>
                        </div>
                        <input type="button" className={styles.btn_write} value="등록" onClick={handleRegister} />
                        <input type="button" className={styles.btn_cancel} value="취소" onClick={goBack} />
                    </form>		
                </div>
            </div>
